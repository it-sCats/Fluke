...
class eventList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      child: FutureBuilder(//باش نبنو الداتا الي بنجيبوها من قاعدة البيانات نحتاجو نحطوها في الفيوتشر بيلدر
          future: EventQuery,
              ...
              if (!snapshot.hasData || snapshot.data.length == 0) {
                ...
                return ListView.builder(
                  reverse: defaultTargetPlatform == TargetPlatform.android ||
                          defaultTargetPlatform == TargetPlatform.iOS//خاصية يحتاجها الموبايل ليعرض الليستة بشكل صحيح
                      ? true
                      : false,
                  scrollDirection:
                      defaultTargetPlatform == TargetPlatform.android ||
                              defaultTargetPlatform == TargetPlatform.iOS//حتى يتم التمرير بالجنب في الموبايل
                          ? Axis.horizontal
                          : Axis.vertical,
                  itemBuilder: (context, index) {
                    var eventData = snapshot.data![index];

                    return event(//ويدجيت خاصة بالكارت الخاص بالحدث يتم تمرير البيانت التي تم إحضارها من قاعدة البيانات إليها
