...
class _recdirectRoleState extends State<recdirectRole> {
  ...
  void _checkRole() async {
    ...
    SharedPreferences userTypeShared = await SharedPreferences//عند تسجيل المستخدم يتم حفظ نوعه في قاعدة البيانات المحلية هذه
    ...
    setState(() {
      role = snap!['role'];//هنا نقوم بإسناد قيمة الحقل للمتغير الذي قمنا بإنشاءه في البداية

    });
    if (role == 'user') {//المستخدم العادي يتم توجيهه للصفحة الرئيسية
      NavigateNext('/home');//دالة تقوم تأخذ إسم الروت وتنقل المستخد له

    } else if (role == 'admin') {//والأدمن يتم توجييه للداشبورد
      NavigateNext('/dashboard');
    }
  }

  void NavigateNext(String RouteName) {//دالة الانتقال
    Timer(Duration(milliseconds: 500), () {//بعد مدة 500 ملي ثانية يتم الانتقال للصفحة المطلوبة
